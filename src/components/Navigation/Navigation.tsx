        ...
        <header ref={ref} className="navigation">
            <div className="navigation-inner">
                ...
                <nav>
                    {NAV_ITEMS.map((item, idx) => (
                        <LenisLink key={idx} to={item.link} className={`nav-link ${item.style === 'button' ? "nav-button" : ""}` }>
                            <p className="nav-text">{item.text}</p>
                        </LenisLink>
                    ))}
                </nav>

                <div className={`navigation-mobile_btn ${mobileOpen ? 'mobile_open': ''}`} id="mobile_nav_btn" onClick={() => setMobileOpen(!mobileOpen)}>
            ...
            <div className={`mobile_nav ${mobileOpen ? 'open' : ''}`}>
                <div className="mobile_nav-wrapper">
                    ...
                    {NAV_ITEMS.map((item, idx) => (
                       <LenisLink key={idx} to={item.link} className={`mobile_nav-link ${item.style === 'button' ? "mobile_nav-button" : ""}` } onClick={closeMenu}>
